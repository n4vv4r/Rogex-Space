	...
	async createEarth(radius = 1.0) {
		...
		const dayMat = new this.THREE.ShaderMaterial({
			uniforms: {
				...
				nightEmissiveColor: { value: new this.THREE.Color(0xffeaa0) },
			...
			fragmentShader: `
				...
				void main() {
					...
					float nightLightMask = smoothstep(0.02, 0.12, nightLum);
			...
			float nightEmit = pow(nightLum, 1.2) * nightLightMask;
			vec3 emissiveTint = vec3(1.0, 0.92, 0.7); // warm city light color
			vec3 nightLightColor = emissiveTint * nightIntensity * nightFactor * nightEmit;
